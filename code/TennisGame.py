# -*- coding: utf-8 -*-
"""
Created on Wed Mar 17 12:19:00 2021
...
class TennisGame_processor(Video):
    ...
    def get_frames(self,start_frame,f,frame_num,transform = lambda x:x):
        ...
        i = 1
        while i<frame_num and cur_frame < self.total_frame:
    ...
    @show_time
    def identify(self,start_time=0,end_time= None,f = 25,batch_size=30,save_path = None) -> np.ndarray:
        print('identifying ...\n')
    ...
    def play_match(self,cls_file = None,add_cls_info = True,write = True):
...
# 制作索引！比赛的索引，点击某个回合就能跳到那个回合，或者加进度条、最后打分排名出最好的回合
# 可以实时展示镜头的分类



video_path = r'D:\video\full\2019WTA印第安维尔斯站女单决赛科贝尔VS安德莱斯库 英文录播.ts'
# video_path = r'D:/video/demo_round.avi'

model_path = r'../model/pg_idf_best.pkl'
cls_file = r'../class_48.npy'
# cls_file = None

tp = TennisGame_processor(video_path,model_path,cls_file)

# tp.random_frame(True)
# tp.play_match(cls_file)
tp.clip_rounds(r'.\rounds')
# clss = tp.identify((0,48,0),(1,0,0),25,120,r'./class_48.npy')
# tp.play_match()

# rounds_time,rounds_frame = tp.clip_match_to_rounds()

# tp.play_match(r'./class_48.npy')
